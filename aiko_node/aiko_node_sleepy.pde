...
#define DEFAULT_TRANSMIT_RATE     60  // seconds
#define HAS_LCD
...
void loop() {
  ...
  if (f_wdt==1) {   // wait for timed out watchdog / flag is set when a watchdog timeout occurs
    ...
    if (skipCount == 10) {
      ...
      delay(2000);    // to allow the send of the data over serial
...
#define CALIBRATION  30  // 10 mV per Amp, analog reference = internal (1 VDC)
