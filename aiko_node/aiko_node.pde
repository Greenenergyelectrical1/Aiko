...
#ifdef IS_PEBBLE
#define DEFAULT_NODE_NAME "ard_pro_1"
#define HAS_LCD
...
#define TRANSMIT_PERIOD 30  // seconds
...
#ifdef IS_STONE
// Analogue Input pins
#define PIN_CURRENT_SENSOR_A  0 // Stone (electrical monitoring)
#define PIN_CURRENT_SENSOR_B  1 // Stone (electrical monitoring)
#define PIN_CURRENT_SENSOR_C  2 // Stone (electrical monitoring)

#define PIN_VOLTAGE_SENSOR_A  3 // Stone (electrical monitoring)
#define PIN_VOLTAGE_SENSOR_B  4 // Stone (electrical monitoring)
#define PIN_VOLTAGE_SENSOR_C  5 // Stone (electrical monitoring)
...
//  Events.addHandler(temperatureSensorHandler, 1000 * TRANSMIT_PERIOD);
  Events.addHandler(inductiveSensorHandler, 1 * TRANSMIT_PERIOD);
#endif
}
...
void voltageSensorHandler(void) {
  ...
  voltageValue = (voltageRaw/1024) * 5.2;
...
/* --------------------------------------------------------------------------*/
// The below senses the jaycar type inductive loop sensor


byte inductiveSensorInitialized = false;
int val1 = 0;
int val2 = 0;
...
float total_rms1 = 0;
float total_watts = 0;
float total_watt_hours = 0;

void inductiveSensorInitialize(void) {
  inductiveSensorInitialized = true;
  startTime = millis();
}

void inductiveSensorHandler(void) {
    
  if (inductiveSensorInitialized == false) inductiveSensorInitialize();
  ...
  while (reading_cnt <= SAMPLE_COUNT) {
    ...
    // Lets sample the analog sensor for the sample size
    // Note, this current... version only uses a single input.
    for (int cnt=0; cnt < SAMPLE_SIZE; cnt++) {
      //val1 = analogRead(PIN_CURRENT_SENSOR_A);
      //val2 = analogRead(PIN_CURRENT_SENSOR_B);
      //val3 = analogRead(PIN_CURRENT_SENSOR_C);
      //Serial.println(val1);
      
      // Do the RMS on the sample
      rms_val1 = rms_val1 + sq((float) analogRead(PIN_CURRENT_SENSOR_A));
      //rms_val2 = rms_val2 + sq((float)val2);
      //rms_val3 = rms_val3 + sq((float)val3);
    }
    
    // Then root the sum of the squares
    rms_val1 = sqrt(rms_val1 / (SAMPLE_SIZE) );
    //rms_val2 = sqrt(rms_val2 / (SAMPLE_SIZE/2) );
    //rms_val3 = sqrt(rms_val3 / (SAMPLE_SIZE/2) );
    
    // Accumulate...
    total_rms1 = total_rms1 + rms_val1;
    
    // Increment our counter.  Nice wording eh?
    reading_cnt++;
    
    if (reading_cnt == SAMPLE_COUNT  ) {
      // BINGO!  Time to then send 'er
      
      // Work out the end time for the total time
      endTime = millis();
      totalTime = endTime - startTime;
      
      // Set the start time for next time around...
      startTime = millis();
      
      // We need to calcualte the average over the sampling and send.
      // Note, 48 is the magic calibration figure....
      ...
      // Beacause there is noise... :(
    
      if ( average_rms1 < 10) average_rms1 = 0;
     
      total_watts = average_rms1;
      total_watt_hours = total_watts * (totalTime/1000)/3600;
      
      // Post to the bufferz
      globalString.begin();
      globalString  = "(power ";
      globalString += total_watts;
      globalString += " W)";
      sendMessage(globalString);
      
      globalString.begin();
      globalString  = "(energy ";
      globalString += total_watt_hours;
      globalString += " Wh)";
      sendMessage(globalString);
...
/* --------------------------------------------------------------------------*/
// The below senses the honeywell type hall effect sensor 
...
//long timer[SAMPLE_SIZE];
  ...
  for (int index = 0;  index < SAMPLE_SIZE;  index ++) {
    timer = micros();
//  timer[index] = micros();
    sample = analogRead(PIN_CURRENT_SENSOR_A);
