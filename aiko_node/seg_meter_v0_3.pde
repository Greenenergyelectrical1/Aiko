...
#define DEFAULT_NODE_NAME "seg_meter_7"
...
#define CALIBRATION  20  // SCT-013-30 amp, 1v output
...
void segMeterHandler() {
  ...
  // A short delay to get things  to settle.
  //delay(150);
  ...
  for (int channel = 0; channel < CHANNELS; channel ++) {
    ...
    if (timeNow > wattHoursTime[channel]) {  // Sanity check, in case millis() wraps around
      duration = timeNow - wattHoursTime[channel];
