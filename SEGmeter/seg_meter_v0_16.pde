...
#define DEFAULT_NODE_NAME "V12011_062"
...
#define CHANNELS          3   // Current sensor(s)
#define SAMPLES        1000   // Samples to take
#define AVERAGES          6  // Number of RMS values to average
...
void setup() {
    ...
    // no ref means that it's at the 5Volts, perfect for the voltage handler.
      ...
      analogReference(INTERNAL1V1);  // ATmega1260
      myAref = 1.1;
    #endif
}
...
void loop() {
    ...
    //nodeHandler();
    powerOutputHandler();
    // The temperature sensor handler is called twice to work around a bug in the handler
    
    // Also misc measurements are sent on their own globa string after the power output.
    globalString.begin();
...
void temperatureSensorHandler(void) { // total time: 33 milliseconds
    ...
    if (oneWireInitialized) {
        ...
        // Now using a higer level global string
        // globalString.begin();
